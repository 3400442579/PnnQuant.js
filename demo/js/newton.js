...
const useState = context => context.Provider.value;
...
class EquationEditor extends preact.Component {	
	...
	onChange = e => {
		...
		const {setState} = useState(AppContext);
		setState({[id]: value});
	}
	onClear = e => {
		const {setState} = useState(AppContext);
...
class App extends preact.Component {
	...
	setData = data => {
		if(data.hasOwnProperty("workouts") && data.workouts.length)
			this.setState(prevState => {
				return {
					...prevState,
					workouts : data.workouts.concat(prevState.workouts)
				}
			});
		else
			this.setState(data);
	...
	render() {
		...
		return preact.Fragment(AppContext.Provider);
	}
}
